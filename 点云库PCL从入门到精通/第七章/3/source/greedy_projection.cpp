...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    print_error(
        "Syntax is: %s input.pcd -w 640 -h 480 -cx 320 -cy 240 -fx 525 -fy 525 "
        "-type 0 -size 2\n",
        argv[0]);
  ...
  tri->setTriangulationType(
      (pcl::OrganizedFastMesh<pcl::PointWithRange>::TriangulationType)type);
  tri->reconstruct(triangles);
  boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(
      new pcl::visualization::PCLVisualizer("点云库PCL从入门到精通"));
